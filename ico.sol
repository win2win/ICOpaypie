    ...
    contract Crowdsale is SafeMath, Pausable, PullPayment {
        ...
        function prepareRefund()  minCapNotReached internal returns (bool){
            ...
            if (backers[msg.sender].refunded ) revert();
            uint value = backers[msg.sender].PPPSent;

            if (value == 0) revert();           
            if (!ppp.burn(msg.sender, value)) revert();
            uint ETHToSend = backers[msg.sender].weiReceived;
            backers[msg.sender].refunded = true;        
            if (ETHToSend > 0) {
    ...
    contract PPP is ERC20, SafeMath, Ownable {
        ...
        modifier onlyUnlocked() {
            if (msg.sender != crowdSaleAddress && msg.sender != preSaleAddress && locked) revert();
            _;
        }
        ...
        function PPP(address _crowdSaleAddress, address _presaleAddress) {
            ...
            balances[_presaleAddress] = 6500000 * multiplier;
